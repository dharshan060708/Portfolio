...
export function CustomCursor({ enabled = true }: CustomCursorProps) {
  ...
  return (
    ...
    >
      {/* Outer subtle ring */}
      <motion.div
        className="w-8 h-8 rounded-full border border-[#D4AF37]/50 pointer-events-none -translate-x-1/2 -translate-y-1/2"
        animate={{
          scale: isHovered ? 1.5 : 1,
          borderColor: isHovered ? '#E6C65C' : 'rgba(212, 175, 55, 0.4)',
        }}
